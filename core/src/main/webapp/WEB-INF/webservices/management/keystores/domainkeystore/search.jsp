...
<div class="page-header">
	<h1><spring:message code="label.management.keystores.searchDomainKeyStore" />
		<small></small>
	</h1>
</div>
<%-- NAVIGATION --%>
<div class="well well-sm" style="display:inline-block">
	<span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>&nbsp;<a class="" href="${pageContext.request.contextPath}/webservices/management/keystores/domainkeystore/create"   ><spring:message code="label.event.create" /></a>
|&nbsp;&nbsp;</div>
...
<script type="text/javascript">
	function addModal(confirmationMessage, formID) {
		$("body").append('<div class="modal fade" id="confirmModal-"' + formID +'>
				...
				<div class="modal-content">
					...
					<div class="modal-footer">
						...
						<a class="btn btn-danger"
							onClick="javascript:$("#' + formID '").submit();">
...
<div class="modal fade" id="deleteModal">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p><spring:message code = "label.management.keystores.searchDomainKeyStore.confirmDelete"/></p>
      </div>
      <div class="modal-footer">
...
<div class="col-sm-2 control-label"><spring:message code="label.DomainKeyStore.name"/></div> 

<div class="col-sm-10">
	<input id="domainKeyStore_name" class="form-control" type="text" name="name"  value='<c:out value='${not empty param.name ? param.name : domainKeyStore.name }'/>' />
</div>	
</div>		
</div>
...
<c:choose>
	<c:when test="${not empty searchdomainkeystoreResultsDataSet}">
		<table id="searchdomainkeystoreTable" class="table responsive table-bordered table-hover">
			<thead>
				<tr>
					<%--!!!  Field names here --%>
...
<%-- Operations Column --%>
		...
		<form id="deleteselected" action="${pageContext.request.contextPath}/webservices/management/keystores/domainkeystore/search/deleteSelected" style="display:none;" method="POST">
		</form>
		
		<button type="button" onClick="javascript:submitOptions('searchdomainkeystoreTable', 'deleteselected', 'domainKeyStores')">
			<spring:message code='label.management.keystores.searchDomainKeyStore.deleteSelected'/>
		</button>
	</c:when>
	<c:otherwise>
				<div class="alert alert-info" role="alert">
