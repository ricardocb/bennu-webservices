...
@RequestMapping("/webservices/management/keystores/uploadcertificate")
public class UploadCertificateController extends WebservicesBaseController {
    ...
    public String uploadcertificateToCancel(@PathVariable("oid") DomainKeyStore domainKeyStore, @RequestParam(
            value = "certificateAlias", required = false) String alias,
            @RequestParam(value = "certificateFile", required = false) MultipartFile certificateFile, Model model) {

        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        try {
            StreamUtils.copy(certificateFile.getInputStream(), byteArrayOutputStream);
        } catch (IOException e) {
            e.printStackTrace();
        }
        byte[] content = byteArrayOutputStream.toByteArray();
        addCertificateToKeyStore(domainKeyStore, alias, content);
    ...
    public String uploadcertificateToCancel(@PathVariable("oid") DomainKeyStore domainKeyStore, Model model) {
        return "redirect:/webservices/management/keystores/domainkeystore/read/" + domainKeyStore.getExternalId();
    }

    @Atomic
    private void addCertificateToKeyStore(DomainKeyStore domainKeyStore, String alias, byte[] certificateContent) {
        domainKeyStore.getHelper().addCertificate(alias, certificateContent);
    }
}
